...
class SignalHandlersTests(TestCase):
    ...
    def test_handle_grade_updated(self, user, grade, send_grade_mock):
        ...
        send_grade_mock.assert_called_once_with(user, grade)


@ddt.ddt
@patch('django_lti_tool_provider.signals.LtiUserData.objects.get')
class CalculateAndSendGradeTests(TestCase):
    ...
    def test_send_grade_requests_correct_lti_user_data(self, get_user_data):
        user = Mock()
        get_user_data.assert_not_called()
        _send_grade(user, 0)
        get_user_data.assert_called_with(user=user)
